...
async resetInterception(page) {
await page.setRequestInterception(false);
page.removeAllListeners("request");
await page.setRequestInterception(true);
page.on("request", req => {
const block = ["doubleclick", "googletagmanager", "facebook", "hotjar", "google-analytics", "analytics.js", "fbevents.js"];
...
async withJS(page, fn) {
  const wasEnabled = page.__jsEnabled === true;

  try {
    if (!wasEnabled) {
      await page.setJavaScriptEnabled(true);
      page.__jsEnabled = true;
    }
...
if (this.jobs % this.options.recycleAfter === 0) {  
  console.log("♻️  Hard recycling browser...");  
...
try {  
  ...
  const isBotChallenge = html.includes("cf-chl-bypass") ||  
...
getLogo($, base) {
let src = $("img[src*='logo'], img[alt*='logo'], .logo img, [class*='logo'] img").first().attr("src") || "";
...
const contactSection = $("section:contains('Contact'), div:contains('Contact'), #contact, .contact").not('footer').text();  
...
const validEmails = [...mails].filter(email => {  
  ...
  const blocklist = [  
    ...
    'your@', 'name@', 'user@', 'info@example', 'support@example',  
    'contact@example', 'hello@example', 'admin@example'  
  ];  
...
$("a").each((_, el) => {  
  ...
  if (href.includes("youtube.com") && !socials.youtube)  
    socials.youtube = href;  
  if (href.includes("github.com") && !socials.github)  
    socials.github = href;  
});  
...
// CMS detection  
if (html.includes('wp-content') || html.includes('wp-json') || $('meta[name="generator"]').attr('content')?.includes('WordPress'))  
  tech.push('WordPress');  
...
detectBusinessType($, html) {
let text = $("body").text().toLowerCase();
let title = $("title").text().toLowerCase();
let description = $("meta[name='description']").attr("content")?.toLowerCase() || "";
let fullText = `${text} ${title} ${description}`;
...
if ((fullText.includes('api') && (fullText.includes('docs') || fullText.includes('documentation'))) ||  
    this.findPage($, '', 'docs')) {  
  scores['Developer Platform'] += 40;  
}  
if (fullText.includes('sdk') || fullText.includes('developer')) scores['Developer Platform'] += 20;  
...
if (fullText.includes('dashboard') && fullText.includes('analytics')) scores['B2B SaaS'] += 20;  

// Consumer SaaS signals  
if (fullText.includes('free trial') && !fullText.includes('enterprise')) scores['Consumer SaaS'] += 25;  
if (fullText.includes('sign up') && fullText.includes('month')) scores['Consumer SaaS'] += 20;  
  
// E-Commerce signals  
if (fullText.includes('add to cart') || fullText.includes('shop now')) scores['E-Commerce'] += 50;  
if (fullText.includes('checkout') || fullText.includes('free shipping')) scores['E-Commerce'] += 30;  
if ($('[class*="product"]').length > 5) scores['E-Commerce'] += 20;  

// Service Business signals  
if (fullText.includes('book appointment') || fullText.includes('schedule consultation')) scores['Service Business'] += 40;  
if (fullText.includes('contact us') && fullText.includes('quote')) scores['Service Business'] += 20;  
...
if (fullText.includes('portfolio') &&   
    fullText.includes('clients') &&  
    (this.findPage($, '', 'pricing') || this.findPage($, '', 'contact'))) {  
  scores['Agency'] += 35;  
}  
if (fullText.includes('case studies') || fullText.includes('our work')) scores['Agency'] += 25;  

// Media/Blog signals  
if ($('article').length > 3) scores['Media/Blog'] += 40;  
if (fullText.includes('subscribe') && fullText.includes('newsletter')) scores['Media/Blog'] += 20;  
...
const isBotChallenge = html.includes("cf-browser-verification") ||   
                      html.includes("Just a moment");  
...
if (data.email) {  
  ...
  if (data.email.includes('contact') ||   
...
for (let i = 0; i < urls.length; i += batchSize) {  
  ...
  for (let j = 0; j < batchResults.length; j++) {  
    ...
    if (result.status === 'fulfilled') {  
      ...
      if (screenshotHighPriority && data.leadScore >= 70) {  
        try {  
      ...
      results.push(data);  
      console.log(`✅ [${i + j + 1}/${urls.length}] ${batch[j]} | Score: ${data.leadScore} | Confidence: ${data.confidence}%`);  
    } else {  
      console.error(`❌ [${i + j + 1}/${urls.length}] ${batch[j]}: ${result.reason.message}`);  
      results.push({   
        website: batch[j],   
...
// Export by niche
async exportByNiche(results, outputDir = './exports') {
...
async save(data, file = "output.json") {
await fs.writeFile(file, JSON.stringify(data, null, 2));
console.log(`Saved to ${file}`);
}

async saveCSV(dataArray, filename = "output.csv") {
if (!dataArray || dataArray.length === 0) return;

const headers = [  
  "Website", "Name", "Business Type", "Lead Score", "Priority", "Confidence",  
  "Email", "Phone", "Twitter", "LinkedIn", "Technologies"  
];  
...
// module.exports = WebsiteIntelligenceEngine;
// ← keep all the class code above...
...
//     const result = await engine.scrape("https://vercel.com");
...
//     console.log("Business    :", result.businessType);
//     console.log("Lead Score  :", result.leadScore, `(${result.priority})`);
//     console.log("Confidence  :", result.confidence + "%");
//     console.log("Email       :", result.email || "(not found)");
//     console.log("Technologies:", result.technologies.join(", ") || "-");
    
//     // Optional: see full result
